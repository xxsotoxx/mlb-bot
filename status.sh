...
if docker service ls | grep -q ${SERVICE_NAME}_mlb-bot; then
    ...
    docker service ps ${SERVICE_NAME}_mlb-bot
else
...
if docker service ls | grep -q ${SERVICE_NAME}_postgres; then
    echo -e "${GREEN}[PostgreSQL]${NC} - Running"
    docker service ps ${SERVICE_NAME}_postgres
else
...
echo "-----------------------------------"
docker service logs ${SERVICE_NAME}_mlb-bot --tail 10

echo ""
echo "Puertos Expuestos:"
echo "-----------------------------------"
docker ps --filter "name=${SERVICE_NAME}" --format "table {{.Names}}\t{{.Ports}}"
